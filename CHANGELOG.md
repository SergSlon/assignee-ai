...
- Git history at commits `312ec5e`, `0d838c0`, `9f45061`, `11c4d54`, `feb12f1` still contains `112233445566`. Full remediation requires `git filter-repo` + force-push to rewrite history — destructive and not authorized. Safe interim posture: HEAD is clean from the Epic 85 commit forward, the repo is PRIVATE today, and a one-time history scrub should happen before the repo's visibility flips to public (v0.2 release). Flagged in the feedback memory as the deferred decision.
