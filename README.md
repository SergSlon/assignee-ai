...
| :----------------- | :---------------------------------------------------- | :-------------------- | :--------------------------------------------------- |
| `bedrock-dev-user` | `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY`         | `AssigneeAiPocPolicy` | Bedrock invocation                                   |
| `aws-mcp-user`     | `MCP_AWS_ACCESS_KEY_ID` / `MCP_AWS_SECRET_ACCESS_KEY` | `AssigneeAiMcpPolicy` | CCAPI, CFN schema, pricing, IAM, billing MCP servers |
...
> No wildcards on `Action` — all permissions follow least-privilege.
