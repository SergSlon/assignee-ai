...
| `status_poller`          | Polls CloudControl until terminal state (SUCCESS / FAILED). Extended timeouts for RDS, ELBv2, NAT Gateway. 503 / Throttling responses retry with exponential backoff + jitter (5 retries, 60s cap per delay); first 503 no longer hard-fails |
