...
A self-contained rules library with **185 BP rule YAML files** tracked in `manifest.json` across 22 AWS service directories (`apigateway/`, `autoscaling/`, `cloudfront/`, `cloudwatch/`, `dynamodb/`, `ec2/`, `ecr/`, `ecs/`, `efs/`, `elbv2/`, `events/`, `iam/`, `kms/`, `lambda/`, `logs/`, `rds/`, `s3/`, `secretsmanager/`, `sns/`, `sqs/`, `ssm/`, `vpc/`). Each directory contains YAML files — one per best practice. The `src/` directory ships the tiny TypeScript runtime that loads, validates, and evaluates those files.
