...
A self-contained rules library. Each AWS service (`ec2/`, `rds/`, `s3/`, `iam/`, `lambda/`, `vpc/`, `elbv2/`, `ecs/`, `ecr/`, `dynamodb/`, `sns/`, `sqs/`, `logs/`, `cloudwatch/`, `secretsmanager/`, `ssm/`, `apigateway/`, `autoscaling/`) has its own directory containing YAML files — one per best practice. The `src/` directory ships the tiny TypeScript runtime that loads, validates, and evaluates those files.
