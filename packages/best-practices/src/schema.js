import { z } from "zod";
import { BP_SEVERITY, BP_CATEGORY, BP_CHECK_TYPE, BP_FIX_TYPE, } from "./types.js";
...
export const bestPracticeSchema = z
    ...
    id: z
        .string()
        .regex(/^BP-[A-Z0-9]+-\d{3}$/, "BP ID must match format BP-{SERVICE}-{NNN}"),
    ...
    message: "fixType 'auto' requires autoFixable=true and a desiredStatePatch",
})
    .refine((bp) => {
    if (bp.fixType === "interactive") {
        return (bp.interactiveOptions != null && bp.interactiveOptions.length > 0);
    }
    return true;
