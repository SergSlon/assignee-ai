/**
 * BP manifest integrity — SHA-256 hashing + freshness only.
 *
 * Story 50-3 deleted the `integrity/` subtree (GPG signing layer was
 * supply-chain theatre). The in-tree SHA-256 manifest validation that
 * `scripts/generate-manifest.ts` and the CI gate use stays here as a
