...
export function getField(obj, path) {
    ...
    for (const segment of segments) {
        ...
        if (bracketMatch) {
            ...
            const container = current[fieldName];
            ...
            if (/^\d+$/.test(bracketKey)) {
                ...
                const found = container.find((item) => typeof item === "object" &&
                    item !== null &&
                    item["Key"] === bracketKey);
                current = found
                    ? found["Value"]
 ...
 */
function checkPasses(checkType, fieldValue, expectedValue) {
    switch (checkType) {
        case "equals":
        ...
        case "greater_than": {
            ...
            if (Number.isNaN(numField) || Number.isNaN(numExpected))
                return false;
 ...
 */
export function matchesTrigger(trigger, context) {
    // resourceType filter — if set and does not match, skip
    if (trigger.resourceType !== undefined &&
    ...
    if (trigger.intentKeywords !== undefined &&
        trigger.intentKeywords.length > 0) {
        if (!context.userIntent)
            return false;
        const lowerIntent = context.userIntent.toLowerCase();
        const hasMatch = trigger.intentKeywords.some((kw) => lowerIntent.includes(kw.toLowerCase()));
        if (!hasMatch)
            return false;
    }
    // patternId — check exact match
    if (trigger.patternId !== undefined) {
        if (trigger.patternId !== context.patternId)
            return false;
    }
 ...
 */
export function evaluateTriggers(context, practices) {
    const findings = [];
    for (const bp of practices) {
        ...
        if (bp.triggers !== undefined && bp.triggers.length > 0) {
            ...
            const anyTriggerMatches = bp.triggers.some((trigger) => matchesTrigger(trigger, context));
            if (!anyTriggerMatches)
                continue;
        }
        else {
            // No triggers array — fall back to matching by resource_type field
            if (bp.resource_type !== context.resourceType)
                continue;
        }
