...
export const cloudFrontDistributionPlugin: ResourcePlugin = {
  ...
  configHints: [
    ...
    "For SPA / static-site CloudFronts, prefer the static-website compound pattern OR the wizard's spa-website / static-site presets. Both produce S3 + OAC + Distribution + the correct ViewerProtocolPolicy=redirect-to-https default; the compound pattern wires the resources together automatically while the preset gives you an editable JSON starting point. Run `assignee infra plan --help` for the apply flow.",
