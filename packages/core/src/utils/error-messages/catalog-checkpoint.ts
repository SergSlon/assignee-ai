...
export const CHECKPOINT_ERROR_MESSAGES: Record<string, ErrorMessageEntry> = {
  [ErrorCode.CHECKPOINT_NOT_FOUND]: {
    ...
    howToFix:
      "Run `assignee infra plan` first to create a checkpoint, then use `assignee infra apply --checkpoint <path>` to apply it.",
  },
  [ErrorCode.CHECKPOINT_EXPIRED]: {
    ...
    howToFix:
      "Run `assignee infra plan` to generate a fresh plan, then apply it.",
  },
  [ErrorCode.CHECKPOINT_INVALID]: {
    ...
    howToFix:
      "Delete the corrupted checkpoint and run `assignee infra plan` to create a new one.",
  },
};
