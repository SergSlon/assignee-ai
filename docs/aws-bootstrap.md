...
The `assignee dev setup` command automates IAM user and policy creation. Run it once with admin/root credentials:
...
is already set in the environment, `assignee dev setup` forwards it alongside the
access key pair — useful for short-lived STS or assumed-role credentials.
The command is idempotent — safe to re-run.

> **Session-token minimum length.** Any session token Assignee accepts must
> be at least 100 characters long; STS-issued tokens are typically
> 100–1000+ characters, and shorter values are almost certainly malformed.
> See `packages/core/src/config/aws-credentials.ts:157`
...
assignee dev init --profile my-sso-profile
```

The `--profile` flag on `assignee dev init` sets `AWS_PROFILE` for all
...
> **Existing users:** re-run `assignee dev setup` to pick up the latest IAM updates
> (`s3:ListBucketVersions`, `s3:DeleteObjectVersion`, `ec2:DescribeAddresses`,
> `iam:GetPolicy`) and the operator A/B policy split. The operator user
...
> **Automated:** Tasks 1–3 are now handled by `assignee dev setup`. The manual steps below are kept for reference only.
...
# Replace the literal `123456789012` in the Resource ARN below with your
# actual 12-digit account ID — the placeholder-ARN preflight guard
# rejects the canonical 123456789012 placeholder at runtime.
...
## Task 3 — Enable Bedrock Invocation Logging

> **Automated:** See Task 1 note above.

```bash
# Replace `123456789012` in the roleArn below with your actual 12-digit
# account ID — the placeholder-ARN preflight guard rejects the
# canonical 123456789012 placeholder at runtime.
...
## Task 4 — IAM Users and Policies (automated by `assignee dev setup`)

The 3 IAM users and their policies are created automatically by `assignee dev setup`.
...
# `@assignee/core` is ESM-only (`"type": "module"`), so this uses dynamic import:
node --input-type=module -e "import('@assignee/core').then(({ operatorPolicy, readerPolicy, auditorPolicy }) => {
...
## Task 5 — Set GitHub Actions Secret (optional, internal CI use)

The `BEDROCK_LOGGING_VERIFIED` GitHub Actions secret is consumed by the
project's internal CI workflows to gate Bedrock-touching jobs. Setting it
is **optional** and only relevant if you fork the repo and run the same
CI pipeline:
...
- [x] 3 IAM users created via `assignee dev setup`: `assignee-operator`, `assignee-reader`, `assignee-auditor`
- [x] Managed policies attached (5 total: 3 on the operator user, 1 on the reader user, 1 on the auditor user): `AssigneeOperatorPolicy`, `AssigneeOperatorServicesAPolicy`, `AssigneeOperatorServicesBPolicy` (all three on the operator user), `AssigneeReaderPolicy` (on the reader user), `AssigneeAuditorPolicy` (on the auditor user)
- [x] Access keys written to `.env`
- [ ] `BEDROCK_LOGGING_VERIFIED=true` secret set in GitHub Actions (optional, internal CI use)
...
- NFR-13: No wildcard IAM permissions on role-management actions
  (`iam:CreateRole`, `iam:AttachRolePolicy`, `iam:PassRole`,
  `iam:PutRolePolicy`) — see
  `packages/core/src/config/iam-policies/operator.ts` for the
  resource-scoping and `iam:PassedToService` condition that
  enforces this.
