...
> **Note for this build.** The alerting infrastructure described below
> (webhooks, sticky issues, on-call escalations) is wired into the
> workflows but the supporting repo secrets are not configured for this
> course-submission build — the webhook channels are unset, so alerts
> resolve to the GitHub-issue path or no-op. The descriptions below
> document the design intent for future productisation.
...
| Prettier format check                           | Every PR + push                   | **Yes**                                                       | `.github/workflows/ci-core.yml`                     | Catches `--no-verify` bypass of the pre-commit `lint-staged` formatter; `pnpm exec prettier --check "**/*.{ts,tsx,json,md}"`                                                               |
...
| `scripts/audit-action-pins.ts`                  | Every PR + push                   | **Yes**                                                       | `.github/workflows/ci-core.yml`                     | Asserts every `uses:` ref in `.github/workflows/*.yml` and `.github/actions/*/action.yml` is pinned to a 40-char SHA                                                                       |
| `scripts/audit-secrets-inherit.ts`              | Every PR + push                   | **Yes**                                                       | `.github/workflows/ci-core.yml`                     | Asserts no workflow uses `secrets: inherit` — every caller declares an explicit `secrets:` block                                                                                           |
| `scripts/audit-overrides.ts`                    | Every PR + push                   | **Yes**                                                       | `.github/workflows/ci-core.yml`                     | Asserts every `pnpm.overrides` key has a sidecar entry in `package.json.overrides-rationale.md`                                                                                            |
| `scripts/audit-codeowners.ts`                   | Every PR + push                   | **Yes** — ENFORCED                                            | `.github/workflows/ci-security.yml` (audit-scripts) | CODEOWNERS exists at repo root, every entry is well-formed, and a `* @<handle>` global catch-all is present                                                                                |
| `scripts/audit-homebrew-pin.ts`                 | Every PR + push                   | **Yes** — ENFORCED                                            | `.github/workflows/ci-security.yml` (audit-scripts) | `--check-template` mode: `homebrew/assignee.rb` uses `${SHA_*}` envsubst placeholders and contains no hardcoded sha256 values (release-time `--check-manifest` mode runs in `release.yml`) |
...
| `scripts/audit-no-suppress.ts`                  | Every PR + push                   | **Yes** — ENFORCED                                            | `.github/workflows/ci-security.yml` (audit-scripts) | Asserts no `\|\| true` masks the exit code of an `assignee` CLI line or `ASSIGNEE_OUTPUT` capture in `.github/actions/*/action.yml`                                                        |
| `scripts/audit-patterns-cleartext.ts`           | Every PR + push                   | **Yes** — ENFORCED                                            | `.github/workflows/ci-security.yml` (audit-scripts) | Scans `~/.assignee/memory/patterns.json` for credential-like strings using the `SENSITIVE_KEY_NAMES` allowlist (exits 0 when file absent)                                                  |
| `ci.yml` (matrix entrypoint)                    | Every PR + push                   | **Yes**                                                       | `.github/workflows/ci.yml`                          | Ubuntu-latest × Node 20 / 22 matrix that delegates the full build/test/coverage/audit suite to `ci-core.yml`                                                                               |
| CodeQL static analysis                          | Every PR + push + weekly schedule | **Yes** (PR + push)                                           | `.github/workflows/codeql.yml`                      | GitHub CodeQL scan for JavaScript/TypeScript security issues                                                                                                                               |
| Cross-OS sweep                                  | Manual + weekly schedule          | **No** — non-blocking surface check                           | `.github/workflows/ci-cross-platform.yml`           | Builds + tests across macOS / Windows / Linux to catch platform-specific regressions; on-demand and weekly cadence                                                                         |
| Daily quality gate                              | Daily schedule                    | **No** — alerting only                                        | `.github/workflows/vacation-quality.yml`            | Runs citation-lint, doc-lint, and pre-close probes on a daily cadence to keep documentation and probe coverage healthy between PRs                                                         |
| Release pipeline                                | Tag push (release)                | **Yes** for the release event                                 | `.github/workflows/release.yml`                     | Build, SBOM, provenance, and dry-run publish; gates the release artefacts                                                                                                                  |
...
window that existed before this gate was enforced.
...
- `.github/workflows/ci-security.yml` — dedicated audit-scripts workflow; enforces `audit-codeowners`, `audit-homebrew-pin`, `audit-iam-policies`, `audit-no-suppress`, `audit-patterns-cleartext`
