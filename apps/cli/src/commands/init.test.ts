/**
 * Tests for `assignee dev init` command.
 *
 * @see Story 18.1, AC #9
 */
...
describe("assignee dev init command", () => {
  ...
  it("config file content matches expected YAML structure", async () => {
    ...
    expect(content).toContain("# Generated by assignee dev init");
...
// `assignee dev init --help` must describe the project config directory as
...
describe("assignee dev init --help (e92-3b3 D-05 help-text half)", () => {
...
describe("assignee dev init --global", () => {
  ...
  it("--global flag triggers global config wizard and writes AssigneeConfig YAML", async () => {
    ...
    const content = await fs.readFile(configPath, "utf-8");
    expect(content).toContain("# Generated by assignee dev init --global");
  ...
  // Note: "no --global flag" behavior is fully tested in the "assignee dev init command" describe block above.
...
describe("assignee dev init non-interactive flags (e92-u.d)", () => {
  const ORIGINAL_STDOUT_IS_TTY = process.stdout.isTTY;
  const ORIGINAL_STDIN_IS_TTY = process.stdin.isTTY;
  ...
  it("non-TTY + no --yes: emits actionable error and exits non-zero", async () => {
    ...
    expect(errBuf).toContain("fix: Re-run with: assignee dev init --yes");
  ...
  // production case where stdin is redirected (`assignee dev init </dev/null`).
  ...
  it("R6/D-01: stdin.isTTY=undefined (piped) trips the guard with exit 1", async () => {
    // Simulate `assignee dev init </dev/null`: stdout still a TTY, stdin
  ...
  // (e.g. `assignee dev init | tee log.txt`). Node also reports
...
describe("assignee dev init --global non-interactive flags (Epic 96 W2 R2)", () => {
  let globalConfigDir: string;
  const ORIGINAL_STDOUT_IS_TTY = process.stdout.isTTY;
  const ORIGINAL_STDIN_IS_TTY = process.stdin.isTTY;
  ...
  it("non-TTY stdin + --global --yes: does NOT hang (D-02 regression)", async () => {
    // Simulate `assignee dev init --global --yes --region us-east-1 </dev/null`.
...
// its own guard above (describe "assignee dev init --help").
describe("assignee dev init D-05 code-emit half (e92-u.d)", () => {
  beforeEach(async () => {
    vi.clearAllMocks();
    const { initCommand } = await import("./init.js");
    (
...
describe("assignee dev init --wizard alias (Epic 96 W2 R4)", () => {
  const ORIGINAL_STDOUT_IS_TTY = process.stdout.isTTY;
  const ORIGINAL_STDIN_IS_TTY = process.stdin.isTTY;
...
// Verifies that `assignee dev init` (no ASSIGNEE_OIDC_ADAPTER) does NOT print
